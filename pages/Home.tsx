...
import LandingPage from '../components/home/LandingPageRestored';
...
const Home: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  // Digital ID should only display for Active users with approved ID Issuance application
  ...
  const dynamicUpdates = useMemo(() => {
    ...
    // 6. Mock News Updates
    MOCK_UPDATES.forEach(upd => items.push({
      ...upd, 
  ...
  const totalOffers = MOCK_DEVICES.length + MOCK_MEDICAL.length + MOCK_LIVELIHOODS.length + 1;
  ...
  return (
    <div className="p-4 md:p-8 space-y-10 md:space-y-16 max-w-7xl mx-auto animate-in fade-in duration-700 pb-32">
      ...
      {/* 1. Hero Highlights */}
      {!isSearching && <HomeHero updates={dynamicUpdates.slice(0, 5)} onClick={setSelectedUpdate} />}
      
      {/* 2. Personal Digital ID Card Section (Fluid Radii) */}
      {!isAdmin && !isSearching && hasDigitalId && currentUser && (
