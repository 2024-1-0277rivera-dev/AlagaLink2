...
const ChatWindow: React.FC<ChatWindowProps> = ({ onClose, targetUser, onBackToList, anchorPosition, isEmbedded = false }) => {
  ...
  const getMunicipalData = () => {
    const devices = MOCK_DEVICES.map(d => ({ id: d.id, name: d.name, type: 'Item', page: 'programs', section: 'Device', image: d.photoUrl }));
    const medical = MOCK_MEDICAL.map(m => ({ id: m.id, name: m.name, type: 'Service', page: 'programs', section: 'Medical', image: m.photoUrl }));
    const livelihood = MOCK_LIVELIHOODS.map(l => ({ id: l.id, name: l.title, type: 'Program', page: 'programs', section: 'Livelihood', image: l.photoUrl }));
    const reports = MOCK_REPORTS.map(r => ({ id: r.id, name: r.name, type: 'Case', page: 'lost-found', section: '', image: r.photoUrl }));
    const pages = [
    ...
    return [...devices, ...medical, ...livelihood, ...reports, ...pages];
  };
