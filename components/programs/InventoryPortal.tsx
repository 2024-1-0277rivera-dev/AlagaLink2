    ...
    <div className="bg-white dark:bg-alaga-charcoal rounded-[24px] overflow-hidden border border-gray-100 dark:border-white/5 group hover:border-alaga-blue transition-all flex flex-col shadow-sm">
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="mb-5 space-y-1.5">
          <div className="flex items-center gap-2 text-[9px] font-bold opacity-50"><i className="fa-solid fa-calendar text-alaga-blue"></i> {item.schedule || 'TBD'}</div>
          <div className="flex items-center gap-2 text-[9px] font-bold opacity-50"><i className="fa-solid fa-location-dot text-alaga-blue"></i> {item.venue || 'Municipal Office'}</div>
        </div>
        <button 
          onClick={() => onApply(type, item.name || item.title, item.id)} 
          className={`w-full py-3.5 rounded-2xl font-black text-sm transition-all ${item.stockCount === 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : `bg-alaga-blue text-white shadow-xl hover:scale-[1.02]`}`}
        >
          {item.stockCount === 0 ? 'Out of Stock' : `Request ${type === 'Device' ? 'Aid' : 'Program'}`}
        </button>
      </div>
    </div>
  );
