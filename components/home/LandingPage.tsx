...
const LandingPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    // Determine the source (explicit sentinel for services without item or a device/medical/livelihood item)
    const sentinel = applyTarget.sentinelServiceId;
    let programType = 'Device';
    if (sentinel) {
      if (sentinel === 'philhealth') programType = 'PhilHealth';
      else if (sentinel === 'medical') programType = 'Medical';
      else if (sentinel === 'livelihood') programType = 'Livelihood';
      else programType = 'Device';
    } else if (selectedService?.id) {
      if (selectedService.id === 'philhealth') programType = 'PhilHealth';
      else if (selectedService.id === 'medical') programType = 'Medical';
      else if (selectedService.id === 'livelihood') programType = 'Livelihood';
      else programType = 'Device';
    } else if (applyTarget.requestedItemId) {
      programType = 'Device';
    }
        ...
        <div className="fixed inset-0 z-[460] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md transition-opacity duration-300 ease-out">
          <div className="bg-white dark:bg-alaga-charcoal rounded-[24px] w-full max-w-3xl p-6 shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-white/10 overflow-y-auto max-h-[80vh] transform-gpu transition-transform duration-300 ease-out">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-2xl font-black">{selectedService.title}</h3>
        ...
        <div className="fixed inset-0 z-[450] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-alaga-charcoal rounded-[32px] w-full max-w-2xl p-8 shadow-[0_40px_100px_rgba(0,0,0,0.5)] border border-white/10">
             ...
             <div className="mt-6 flex items-center gap-4">
                  <button onClick={() => { scrollTo(joinRef); setAboutService(null); setShowLoginPopover(true); }} className="px-6 py-3 bg-alaga-blue text-white rounded-[12px] font-black">Register / Login</button>
